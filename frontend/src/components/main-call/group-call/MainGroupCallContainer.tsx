...
function MainGroupCallContainer() {
  ...
  const getGridClasses = () => {
    switch (groupCallStreams.length) {
      case 2:
        return "grid grid-cols-2 place-items-center gap-1 justify-center";
      case 3:
  ...
  return (
    <div className={`w-full h-full transition-none  ${getGridClasses()}`}>
      {groupCallStreams?.map((stream: MediaStream) => {
        const user = groupCallUsers.find((user) => user.streamId === stream.id);
        return <GroupVideoElement stream={stream} user={user} />;
      })}

      <LocalVideo
        divClassName="relative z-10 shadow-2xl "
